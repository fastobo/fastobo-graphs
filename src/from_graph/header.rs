    ...
    fn from_graph(pv: BasicPropertyValue) -> Result<Self> {
        match pv.pred.as_str() {
            obo_in_owl::AUTO_GENERATED_BY => {
                Ok(HeaderClause::AutoGeneratedBy(Box::new(UnquotedString::new(pv.val))))
            }
            ...
            obo_in_owl::HAS_OBO_FORMAT_VERSION => {
                Ok(HeaderClause::FormatVersion(Box::new(UnquotedString::new(pv.val))))
            }
            obo_in_owl::NAMESPACE_ID_RULE => {
                Ok(HeaderClause::NamespaceIdRule(Box::new(UnquotedString::new(pv.val))))
            }
            obo_in_owl::SAVED_BY => Ok(HeaderClause::SavedBy(Box::new(UnquotedString::new(pv.val)))),
